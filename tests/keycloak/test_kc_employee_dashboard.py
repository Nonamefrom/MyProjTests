...
class TestLoginEmployeeDashboard:
    @allure.title("Запрет авторизации существ. пользователя ЛК Сотрудника с неправильным паролем")
    @allure.id('KeyCloack/EmpDash/№ 1')
    ...
    @allure.title("Запрет авторизации НЕ существующего b2b пользователя ЛК Сотрудника с правильным паролем")
    @allure.id('KeyCloack/EmpDash/№ 2')
    def test_employee_dash_login_b2b_wrong_mail(self, kc):
        kc.emp_dash_auth_form.open().login(WRONG_MAIL, B2B_PASS)
        got_error = kc.emp_dash_auth_form.error_message()
        assert ER.ERROR_TEXT == got_error, f"Expected '{ER.ERROR_TEXT}' but got '{got_error}'"

    @allure.title("Запрет авторизации с л/п internal пользователя в ЛК Сотрудника")
    @allure.id('KeyCloack/EmpDash/№ 3')
    def test_employee_dash_login_b2b_like_internal(self, kc):
        kc.emp_dash_auth_form.open().login(INTERNAL_MAIL, INTERNAL_PASS)
        got_error = kc.emp_dash_auth_form.error_message()
        assert ER.ERROR_TEXT == got_error, f"Expected '{ER.ERROR_TEXT}' but got '{got_error}'"

    @allure.title("Авторизация корректного сотрудника в ЛК Сотрудника")
    @allure.id('KeyCloack/EmpDash/№ 4')
    ...
    @allure.title("Восстановление почты пользователя + кейс несовпадения вводимых новых паролей в ЛК Сотрудника")
    @allure.id('KeyCloack/EmpDash/№ 5')

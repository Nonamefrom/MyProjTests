...
class TestB2BEmployees:
    ...
    def test_unique_phone(self, pages):
        ...
        time.sleep(3)  # Таймер позволяет дождатся отображения страницы и выключения бабла добавленного юзера
    ...
    def test_unique_mail(self, pages):
    ...
    def test_add_and_delete_user(self, driver, pages, steps):
        ...
        steps.tables_steps.find_entity(mail)
        pages.b2b_employee_page.delete_employee().accept_delete_employee()
        #driver.refresh()
        #time.sleep(4)
        check_delete = steps.tables_steps.count_find_entity(mail)
